...
for i=1:length(phi_vec)
    for k=1:360
        ...
        r(i,k)=-ade_mag_curr*cosd(k-phi_curr)+ada(i);
        t(i,k)= alambda_curr+2*ade_mag_curr*sind(k-phi_curr);
        n(i,k)=adi_mag_curr*sind(k-nu_curr);
    end
end
...
for k=1:360
    for index1=1:length(phi_vec);
        r1vec=[r(index1,k), t(index1,k), n(index1,k)];
        for index2=(index1+1):length(phi_vec)
            r2vec=[r(index2,k), t(index2,k), n(index2,k)];
            sep_curr=norm(r1vec-r2vec);
            if sep_curr<minsep
                minsep=sep_curr;
                indexmin=[index1 index2 k];
            end
        end
    end
end
